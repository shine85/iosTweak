...
- 针对特有应用（例如“中国移动”）：分析并猜想其内部自身的 Splash 管理器或视图控制器（如 \`CMSplashManager\`, \`CMSplashViewController\`, \`CMAdManager\`），拦截其拉取或渲染方法。千万不要使用短前缀（如 \`CM\`）去大规模遍历 View。
...
- **成员属性与 @interface 编译约束 (CRITICAL)**：若你在 \`%hook\` 代码块内部使用了 \`self.hidden\`, \`self.view\`, \`self.delegate\`, \`self.window\` 等任何系统 UIView 或 UIViewController 才有的属性，**你必须在文件的顶部为该类提供正确的 \`@interface\` 声明，并强制让其继承自 \`UIView\` 或 \`UIViewController\`！** 例：\`@interface SplashView : UIView @property (nonatomic, weak) id delegate; @end\`。因为如果你不继承，编译器默认它是 \`NSObject\` 从而报出 \`property 'hidden' not found on object of type '__unsafe_unretained id const'\` 的致命错误导致编译中断。如果该类明明是 controller 但是你没声明父类，它就不认识 \`self.view\`。
