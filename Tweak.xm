    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.8 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        for (UIWindow *window in [UIApplication sharedApplication].windows) {
            ...
            if ([className containsString:@"Splash"] || [className containsString:@"Ad"] || 
                ...
                [className containsString:@"CSJSplash"] || window.windowLevel >= UIWindowLevelNormal + 1) {
                window.hidden = YES;
...
%ctor {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        NSLog(@"[AdHook] 中国移动手机营业厅去开屏广告 Tweak 已加载并执行兜底清理");
    });
    
    NSLog(@"[AdHook] 中国移动手机营业厅去开屏广告 Tweak 已加载");
}