...
static void aggressivelyKillSplashViews() {
    ...
    for (UIView *sub in keyWin.subviews) {
        ...
        } else if (CGRectGetWidth(sub.frame) >= CGRectGetWidth(keyWin.frame) * 0.9 && 
                   CGRectGetHeight(sub.frame) >= CGRectGetHeight(keyWin.frame) * 0.9) {
            // 额外处理全屏疑似开屏视图
            if (sub.subviews.count > 0 || [sub isKindOfClass:[UIImageView class]] || [sub isKindOfClass:[UIWebView class]]) {
                NSLog(@"[CMAdBlock] Removed full-screen suspicious view: %@", className);
                sub.hidden = YES;
                sub.alpha = 0.0;
                [sub removeFromSuperview];
            }
    ...
    // 恢复主内容视图(防止误杀)
    if (keyWin.rootViewController && keyWin.rootViewController.view) {
...
@interface CMSplashView : UIView
@end

%hook CMSplashManager
- (void)showSplash { NSLog(@"[CMAdBlock] Blocked CMSplashManager showSplash"); }
- (void)loadSplashAd { NSLog(@"[CMAdBlock] Blocked CMSplashManager loadSplashAd"); }
%end

%hook CMAdManager
- (void)showOpenScreenAd { NSLog(@"[CMAdBlock] Blocked CMAdManager showOpenScreenAd"); }
%end

%hook CMSplashView
- (void)show { 
    NSLog(@"[CMAdBlock] Blocked CMSplashView show"); 
    self.hidden = YES; 
...
%ctor {
    %init(GDTSplashAd=objc_getClass("GDTSplashAd"),
          ...
          CMSplashView=objc_getClass("CMSplashView"));
    
    // 多重延迟 + 周期性强杀，解决开屏页残留问题
    ...
    NSLog(@"[CMAdBlock] 中国移动营业厅去广告 Tweak 已加载 - 加强开屏移除版 (cn.10086.app)");
}