...
- [x] **对象属性访问与 C 函数传参强约束 (v1.1.62)**：修复在 `UIViewController` 等泛对象上可能把 `self` 推断为 `id` 导致找不到 `view` 属性报错的问题；修复自定义 `hookIfExists` 把目标类名直接当符号传参导致的 `unexpected interface name` 拦截报错。现已在后端严格约束 AI 必须进行 `((UIViewController *)self).view` 显式转换，并使用带 `"ClassName"` 双引号的方式传给 C 函数。
