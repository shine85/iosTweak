...
- [x] **进一步修复去广告时 `id const` 和未知类对象点语法的致命崩溃**：日志表明在 Hook `CMSplashViewController` 等未声明完整父类的动态类时，即使为其补充了 `@class`，当使用 `self.presentingViewController` 或 `self.view` 时依然会触发 `property not found on object of type '__unsafe_unretained id const'`。在后端的 Prompt 中彻底强制规定：只要是对被推断为 `id` 的对象操作专属属性，**必须使用显式的接口强转（如 `((UIViewController *)self).view` 或者 `((UIViewController *)self).presentingViewController`）或者消息发送语法**！一劳永逸解决动态 Hook 时所有编译器不认识 `self` 身份导致的编译死机！ (v1.1.90)
...
- [x] 系统级版本号同步：建立了跨 `package.json`、语言包、登录 UI 及 GitHub Actions 的版本号联动机制，当前已同步至 v1.1.90。
