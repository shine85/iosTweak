...
- [x] 系统级版本号同步：建立了跨 `package.json`、语言包、登录 UI 及 GitHub Actions 的版本号联动机制，当前已同步至 v1.1.82。
- [x] **修复 TrollFools 等注入工具显示名为 0.dylib 的断代解析问题**：将云编译阶段产生的 `.dylib` 产物的文件名中版本号分隔符由 `.` 替换为 `_`（下划线），例如 `v0_1`，彻底规避了那些基于 `.` 暴力截断扩展名的注入工具提取版本后缀引发的错误！(v1.1.82)。
