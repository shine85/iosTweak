...
- [x] 系统级版本号同步：建立了跨 `package.json`、语言包、登录 UI 及 GitHub Actions 的版本号联动机制，当前已同步至 v1.1.70。
- [x] **深度打磨 IDE 体验 (Polish & Fixes)**：放大并汉化了“导入”、“复制”及“一键云编译”大按钮。解决了 `/api/build` 缺失导致的 404/JSON 解析错误，交互全面向生产力靠拢 (v1.1.70)。
