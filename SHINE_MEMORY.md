...
- [x] **优化代码与说明文案拆分逻辑，防止解释文本截断与丢失**：由于模型返回的文本常常不按套路出牌（例如直接开头就是代码块，或在代码块结束后追加说明补充、甚至带有 Makefile 配置项等附加代码），原先简单的拆分逻辑常常导致 UI 上的解析只剩下“代码生成完成。”。我们重写了对 Markdown 和各种格式代码块的正则表达式拦截和二次切片逻辑，保证即便是位于 Objective-C 代码块之后出现的所有解释性说明或者关联的构建配置代码，也将连贯无损地展示到页面对话流中，不影响前端主编辑区的纯净。
- [x] **终极强化底层 Prompt：要求一次性提供开屏、信息流、插屏弹窗等【全满贯】式强效杀灭**：针对用户痛点，严禁底层大模型给出“残缺”的骨架代码或者懒惰式的一两行代码。在 `server.ts` 的 `TWEAK_REQUIREMENTS` 中注入了更为暴躁且不可抗拒的防御与攻击规范（v1.1.98版），命令模型必须像顶尖 iOS 安全专家一样预判出所有的广告 SDK（开屏、插屏、Banner），并将其完整包含于首次生成的代码中，最大化降低用户的调试压力。
- [x] **进一步确保生成产物的工程完整性**：针对用户要求再次升级底层 Prompt 指令，强制要求大模型在生成方案时除了代码本身，还必须包含对 `Filter.plist` 的配置指导、`control` 文件的正确构成、Makefile 中 `Foundation` 和 `UIKit` 基础依赖框架的自动声明，以及在 `applicationDidFinishLaunching:` 中的日志探针，确保插件在注入后可以直接观察成效。
- [x] **修复宝塔部署及生产环境 TransformError 构建中断问题**：在上一次更新 Prompt 指令时，意外截断了 Express 请求日志中间件 \`app.use((req, res, next) => {...})\` 的闭合代码块标签，导致 TypeScript (tsx/esbuild) 遇到语法错误从而引发白屏及部署中断。当前已修复并加设了强验证流。
- [x] **进一步细化去广告策略与编译配置约束 (Refine & Secure Hooks)**：针对用户的进阶开发痛点，继续深挖 Prompt 潜力。在 \`Tweak.xm\` 要求中明确增加了全局开屏 \`UIWindow\` 发现后重置焦点（\`resignKeyWindow\`）和隐藏的行为；明确为 CSJ、GDT 等主流开屏广告 SDK 提供标准化的防黑屏 Logos Hook 模板拦截加载；在 \`Makefile\` 构建体系要求中，死守 \`-Wl,-current_version,1.0.0\` 注入项，并强制大模型输出所有可能引用到的类声明 \`>>> 编译安全约束 <<< 强化@interface ClassName : NSObject @end\`，以消除由于指针不明确导致的未知实例编译报错。
- [x] **彻底根治编译阶段 %group 嵌套与符号死锁问题**：用户反馈编译流程报错 \`%group inside a %group\` 且伴随 Logos 预处理器解析卡顿退出（Error 255/2）。原因为 AI 生成代码时不仅遗漏了 \`%end\` 闭合，还直接将上一个版本的 Prompt 命令关键字（如 \`>>> 编译安全约束 <<<\`）作为裸代码违规插入 \`Tweak.xm\` 内。已在后端核心控制域 \`server.ts\` 内补入【绝杀级】的规范命令，强行定义连续 \`%end\` 闭合结构并封杀一切非规则 ObjC 注释符号。
- [x] **集成原生级别的大型模块化去广告引擎 (Surge/Loon/adultraplus适配及动态更新)**：针对用户提议的“规则远程自动更新”需求，我们在代码核心侧的 Prompt `TWEAK_REQUIREMENTS` 中不仅限制了大批量正则导致的 OOM / 卡顿，还特别加入了全新的指令要求大模型生成：在 `%ctor` 中启动专用的后台静默下载更新线程。该线程可在底层应用重载时通过异步网络抓取最新的外部 URL (诸如 `广告平台拦截器` )，拉起之后高速缓存入 `NSCachesDirectory` 并装载至 `NSSet`，如此一来无需重新打包也可做到和源平台同样的**动态自动更新**效果，避免硬编码污染！
- [x] **实现应用内规则管理后台（人工设置介入）**：为了响应用户要求的“后台接口/人工设置”，在系统 Prompt `TWEAK_REQUIREMENTS` 中追加了“人工控制面板”的构建指南。强制模型在面临此类需求时，必须产出诸如“监听摇一摇或三指点击长按手势”的 Hook，从而弹框调出原生的 `UIAlertController` 配置界面，允许用户直接在应用内部更改和保存订阅 URL 等订阅规则到 `NSUserDefaults`。实现了去中心化环境下的“伪后台”人工管控系统。
- [x] **开放全局 AI 拦截规则配置接口 (Settings Tab 规则池)**：为了应对规则源经常变更的情况，已经在前端的 **Settings (设置) 界面** 新增了【远程去广告规则地址 (换行分割)】配置项，并对其做了**UI 高亮加粗**以统一整体设计规范。该配置会自动下发至 Node.js 后端，并在每次 AI 生成 `Tweak.xm` 时作为最高强制上下文（CRITICAL 指令）透传给模型。以后每次生成去广告插件时，AI 均会“默认识别并编译写入这些你自己设定的远程源”，告别死板的硬编码和繁琐的重新打包。
- [x] **新增前端生成时的自定义对话框 (对话意图补全)**：为了解决用户点击“生成LOGOS源码”由于只能传 App Name 导致 AI 无法获取精确意图的痛点，在生成按钮上方新增了一个【自定义对话 / 附加要求 (可选)】的多行输入框。用户现在可以自由对 AI 提出明确指令，例如：“帮我生成小红书的去广告 Tweak，并且把保存无水印也加上”。这部分 `customPrompt` 在传输至服务端时，精确绑定了底层的 Prompt 模板供大模型分析。并优化了聊天框的气泡文本显示方案，当含自定义对话时仅展示“【目标：xxx】+ 自定义内容”，消除语义重复感。
- [x] **优化全局标题层级与面板输入框高度 (UI 体验提升)**：用户反馈“API接口设置”等顶部大型模块标题过大（原先使用了 `text-6xl` 导致视觉压迫感极强），同时“远程去广告规则地址”在界面中显得层级不清且输入框偏短。已全局将各个 Tab（Builder、Guides、Settings 等）的 `h2` 主标题降级为合理的 `text-4xl`，并且将自定义对话及规则配置所在的 `<textarea>` 最小高度统一从 `80px` / `100px` 拉长至更为宽裕的 `150px` 方便粘贴含有大量地址的多行文本。最后，将“远程去广告规则地址”单独抽出为一个与“GitHub自动化配置”平级的 `h3` 标题块（带盾牌 Icon 标识），视觉结构更加工整清晰。
- [x] **彻底修复 Theos 自动构造器报错问题 (Cannot generate an autoconstructor with multiple %groups)**：发现即使修复了嵌套错误，因为 Theos 的 Logos 在面对多 \`%group\` 时，如果文件末尾没有显式编写 \`%ctor\` 会直接抛错。已经在 Prompt 策略中通过极其严厉的警告强制 AI 必须在文件最底部显式编写 \`%ctor { %init(GroupA); %init(GroupB); }\`，防止在云编译中再次引发 \`Error 255\`。
- [x] **修复前端左侧预览区代码渲染缺失及错位**：用户反馈左侧源码区异常变为空白（仅剩 `**Makefile**` 文本），且调试聊天框右侧杂乱无章排布大段异常代码。检测发现服务端对于 Markdown 代码块过滤截取的 `split` 算法过于简单，无法正确处理 `\r\n` 结尾的标识以及多种带有 Makefile 和 Objc 多段落返回体的拦截逻辑。已重构 `server.ts` 提取代码，改为更强大且稳定的基于正交匹配块的方法 `match(/…/i)` ，使得 `Tweak.xm` 被精确切割至左侧编辑区，而相关补充日志被可靠归拢于聊天框气泡，彻底消除文本断层。
- [x] **拦截 Theos 编译期 '%group inside a %group' 致命报错**：检测到 AI 在生成 Logos 特性代码时，经常遗漏对 `%group` 和 `%hook` 块进行闭合配对（少写 `%end`），导致编译阶段爆出“%group inside a %group”的错误。已在后端核心 Prompt 注入严厉禁令，强制模型必须为每一个 `%group` 及其子 `%hook` 补充完整的配对 `%end`，消除不规范的悬空代码区块。
